...
class AutoShutdown():
    def __init__(self, rt):
        ...
        self.setup_frame()
        self.setup_widget()
        self.timer() # decrease timer
        self.root.bind("<space>", self.start_wrapper())
        # End of __init__
    ...
    def setup_widget(self):
        ...
        self.check_restart = tkinter.Checkbutton(self.frame0, variable=self.shutdown_menu, onvalue=2, offvalue=1,  text="재부팅                                                  ")
        ...
        self.check_2 = tkinter.Checkbutton(self.frame2_1, variable=self.timer_menu, onvalue=2, offvalue=1, text="지정한 시각에 종료                                              ")
        ...
        self.lb1_min = tkinter.Label(self.frame1_2, text="분 후 윈도우를 종료합니다.   ")
        ...
        self.lb_info_timer_3 = tkinter.Label(self.menu_frame, text="초", foreground='red')
        self.lb_info_timer_4 = tkinter.Label(self.menu_frame, text="후 종료합니다.")

        self.btn_start = tkinter.Button(self.menu_frame, text="타이머 시작", default='active')
        self.btn_start.bind('<Button-1>', self.start_wrapper())
        self.btn_stop_restart = tkinter.Button(self.menu_frame, text="  정 지  ")
        self.btn_stop_restart.bind('<Button-1>', self.stop_wrapper())
        self.btn_close = tkinter.Button(self.menu_frame, text="  닫 기  ", command=self.root.destroy)
        ...
        self.btn_stop_restart.grid(row=2, column=0, padx=10, pady=3, ipadx=14, sticky ='we')
        self.btn_close.grid(row=2, column=8, padx=10, pady=3, ipadx=14, sticky = 'we')
        ...
        self.lb_info_timer_4.grid(row=1, column=8)

    def timer(self):
        if self.clicked:
            # decrease by 1 second
            self.timer_temp.set(self.timer_temp.get() - 1)
    ...
    def start(self):
        ...
        self.btn_stop_restart.config(text="  정 지  ", default='disabled')
        self.btn_stop_restart.bind("<Button-1>", self.stop_wrapper())
        self.set_timer()

    def stop(self):
        if self.clicked: # clicked -> stop
            self.clicked = False
            self.btn_stop_restart.config(text=" 재시작 ", default='active')
            self.btn_stop_restart.bind("<Button-1>", self.restart_wrapper())

    def restart(self):
        if self.clicked == False: # not clicked -> restart
            self.clicked = True
            self.btn_stop_restart.config(text="  정 지  ", default='disabled')
            self.btn_stop_restart.bind("<Button-1>", self.stop_wrapper())
            self.set_timer()
